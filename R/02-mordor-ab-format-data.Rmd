...
#-------------------------------
table(is.na(d4$childid[d4$antigen=="vsp3"]))
newids <- data.frame(dbsid = d4$dbsid[is.na(d4$childid) & d4$antigen=="vsp3"],childid_recode = paste0("childid recode ",1:19),stringsAsFactors = FALSE)
d4 <- d4 %>%
  left_join(newids, by = "dbsid") %>%
  mutate(
    agem = ifelse(is.na(childid), agemosblood_, agem),
    childid = ifelse(is.na(childid),childid_recode,childid)
    )
...
# cluster C51 is missing 
# treatent information in phase6
# assume it was 183 days later than phase0
d_firsttr$mintrdate[d_firsttr$clusterid=="C51" & d_firsttr$phase == 6] <- as.Date("2015-07-06")+183
d_firsttr <- d_firsttr %>%
  group_by(clusterid) %>%
...
d4 <- left_join(d4,d_firsttr, by = c("clusterid","phase"))
...
d_pre2 <- d_pre %>%
  group_by(clusterid,pyear) %>%
  dplyr::select(clusterid,pyear,wet_start,wet_end) %>%
  slice(1) %>%
  ungroup()
...
d4 <- d4 %>%
  ...
  left_join(d_pre2, by = c("clusterid","pyear")) %>%
  dplyr::select(-pyear)
...
#-------------------------------
d11 <- d10 %>%
  dplyr::select(clusterid,phase,arm,childid,agem,agem_start, agem_start_under6, sex, 
         testdate,
...
dtr2 <- dtr %>%
  ...
  # format cluster names for merge
  mutate(cluster_name = str_to_upper(randomunit999)) %>%
  # limit to relevant vars
  dplyr::select(cluster_name,dupwpid_a,childid=masterperson, phase, agem = agex, female = femalex, trdate, treated, dose=dosex)

#-------------------------------
# grab the community ID variables 
# created above for the morbidity data
# the community names do not match 
# between these data sources (!)
# reinforcing the importance of
# creating a standard ID variable
#
# use individiual child IDs to join 
...
dchids <- d10 %>%
  ungroup() %>%
  dplyr::select(childid, clusterid, morbidity_cluster_name = cluster_name, arm) %>%
  group_by(childid) %>%
  slice(1)

dtr3 <- dtr2 %>%
  left_join(dchids, by=c("childid")) %>%
  ungroup() %>%
  group_by(cluster_name) %>%
  tidyr::fill(clusterid, .direction = "downup") %>%
  tidyr::fill(morbidity_cluster_name, .direction = "downup") %>%
  tidyr::fill(arm, .direction = "downup") %>%
  ungroup() 
#-------------------------------
# print a list of cluster IDs
# and cluster names from the
# treatment data and morbidity
# data, just in case
# it might be useful
#-------------------------------
dtr_cnames <- dtr3 %>%
  group_by(clusterid) %>%
  dplyr::select(clusterid, treatment_cluster_name = cluster_name, morbidity_cluster_name) %>%
  slice(1) 

## COMMENTED OUT FOR PUBLIC FILES
## TO OBSCURE CLUSTER NAMES
## knitr::kable(dtr_cnames) %>% kableExtra::kable_styling(bootstrap_options = "striped")
...
# variables
dtr4 <- dtr3 %>%
  dplyr::select(clusterid,arm,phase,childid,agem,female,trdate,treated,dose)
...
#-------------------------------
summary(dtr4)
write_rds(dtr4,"~/Box/mordor/mordor data/data/final-dbs/mordor-ab-treatment.rds")
write_csv(dtr4,"~/Box/mordor/mordor data/data/final-dbs/mordor-ab-treatment.csv")
