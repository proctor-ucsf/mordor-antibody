...
# data to share
# dropping study IDs and antigens
# not included in the malaria
# and bacteria/protozoa analyses
#-------------------------------
d12 <- d11 %>%
  select(-gwu, -masterperson_mcd,-pf_thicksmear,-pf_density,-pf_gameto) %>%
  filter(!antigen %in% c("pgp3","ct694","nie")) %>%
  mutate(antigenf = factor(antigenf))
summary(d12)
...
#-------------------------------
dtr4 <- dtr4 %>%
  dplyr::select(clusterid_public,arm,phase,childid_public,agem,female,trdate,treated,dose)
```
