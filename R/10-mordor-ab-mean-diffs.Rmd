...
d_mfi_boot <- d2 %>% ungroup() %>% select(clusterid,antigenf,arm,logmfi)
...
mu_mfi_table <- mu_mfi2 %>%
  ...
  select(antigenf, placebo_n, placebo, azithro_n, azithro, diff_ci, permute_p) %>%
  mutate(bh_p = c(bh_p_enterics,bh_p_malaria))

knitr::kable(mu_mfi_table %>% slice(1:10),
             col.names = c("Pathogen, antigen", "N","Mean","N","Mean","Diff. (95% CI)","P-value*","P-value†"), 
             digits = c(0,0,3,0,3,0,2,2),
             align = "lccccccc") %>%
  kable_styling(bootstrap_options = c("striped","condensed"),full_width = TRUE) %>%
  add_header_above(c(" " = 1, "Placebo" = 2, "Azithromycin" = 2, "Difference in log10 IgG" = 1,  " " = 2)) %>%
  footnote(
...
plot_mfi_means_enterics <- ggplot(data = mu_mfi_plot %>% filter(group != "malaria"), aes(x = antigenf, color = arm)) +
  ...
  labs(x = NULL, y = "Mean log10 IgG Luminex response\n(MFI-bg)") +
...
ggsave(here("figures","mordor-ab-diff-mfi-bacteria-protozoa.pdf"),plot_mfi_comp_enterics,device = "pdf",width = 8, height = 3)
```
...
d_labels <- data.frame(group = rep("malaria",2),
                       antigenf=rep("P. falciparum AMA1",2),
                       arm = c("azithromycin","placebo"),
                       mean = rep(2.5,2))
...
ggsave(here("figures","mordor-ab-diff-mfi-malaria.pdf"),plot_mfi_comp_malaria,device = "pdf",width = 8, height = 3)
```
...
knitr::kable(mu_serop_table %>% slice(1:6),
             col.names = c("Pathogen, antigen", "N", "Proportion seropositive","N", "Proportion seropositive","Difference (95% CI)","P-value*","P-value†"), 
             digits = 2,
             align = "lcccccc") %>%
  kable_styling(bootstrap_options = c("striped","condensed"),full_width = TRUE) %>%
  add_header_above(c(" " = 1, "Placebo" = 2,"Azithromycin" = 2, " " = 3)) %>%
  footnote(
    symbol=c("Exact permutation P-value.","Permutation P-value with false discovery rate correction using the Benjamini-Hochberg method")
  )
...
plot_serop_means_enterics <- ggplot(data = mu_serop_plot %>% filter(group != "malaria"), aes(x = antigenf, color = arm)) +
  ...
  labs(x = NULL, y = "IgG seroprevalence (%)\n ") +
...
ggsave(here("figures","mordor-ab-diff-seroprev-bacteria-protozoa.pdf"),plot_serop_comp_enterics,device = "pdf",width = 8, height = 2.5)
```
...
ggsave(here("figures","mordor-ab-diff-seroprev-malaria.pdf"),plot_serop_comp_malaria,device = "pdf",width = 8, height = 2.5)
```
...
# included in each FOI analysis for legend reporting
...
knitr::kable(foi_ests_tab %>% select(antigenf,lambda0c,lambda1c,hrc,hr_p,bh_p) %>% slice(1:6),digits=3,
...
ggsave(here("figures","mordor-ab-scr-hr-bacteria-protozoa.pdf"),plot_scr_comp,device = "pdf",width = 8, height = 2.5)
```


## Malaria FOI table
```{r foi table of results malaria}
knitr::kable(foi_ests_tab %>% select(antigenf,lambda0c,lambda1c,hrc,hr_p,bh_p) %>% slice(7:15),digits=3,
             caption="Average seroconversion rate (SCR) per year estimated from age-structured seroprevalence",
             align = "lcccc",
             col.names = c("Antigens","Placebo SCR (95% CI)","Azithromycin SCR (95% CI)", "HR (95% CI)","P-value*","P-value†"),
             row.names = FALSE)  %>%
...
ggsave(here("figures","mordor-ab-scr-hr-malaria.pdf"),plot_scr_comp_malaria,device = "pdf",width = 8, height = 2.5)
```
