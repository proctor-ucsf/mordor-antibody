...
foi_ests <- foreach(ab=levels(d_foi$antigenf),.combine=rbind) %do% {
    ...
    newd0 <- data.frame(agem=c(min(pd$agem),max(pd$agem)), arm = "sun", dummy=0, clusteridf = "C1")
    newd1 <- data.frame(agem=c(min(pd$agem),max(pd$agem)), arm = "moon", dummy=0, clusteridf = "C1")
...
Create separate figures for enterics and malaria, since the enteric pathogens have so much higher FOI — hard to keep on the same scale
...
```{r scr composite figure enterics, fig.width = 10, fig.height = 3}
plot_scr_comp <- grid.arrange(plot_scr_means_enterics,plot_scr_hr_enterics,ncol=2,nrow=1, widths = c(4,4))

ggsave(here("figures","mordor-ab-scr-hr-enterics.png"),plot_scr_comp,device = "png",width = 9, height = 3)
```
...
```{r scr composite figure malaria, fig.width = 10, fig.height = 3}
plot_scr_comp_malaria <- grid.arrange(plot_scr_means_malaria,plot_scr_hr_malaria,ncol=2,nrow=1, widths = c(4,4))

ggsave(here("figures","mordor-ab-scr-hr-malaria.png"),plot_scr_comp_malaria,device = "png",width = 9, height = 3)
```
