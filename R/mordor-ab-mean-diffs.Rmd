...
#-----------------------------
bh_p_enterics <- p.adjust(permute_mfi$permute_p[1:11],method = "BH")
bh_p_malaria <- p.adjust(permute_mfi$permute_p[12:20],method = "BH")

```

## Enterics MFI summary table
```{r MFI means table enterics}
...
knitr::kable(mu_mfi_table %>% slice(1:11),
             col.names = c("Pathogen, antigen", "Placebo","Azithro.","Difference (95% CI)","P-value*","P-value†"), 
...
## Enterics MFI summary figure

```{r MFI means figure enterics, warning=FALSE}
#------------------------------
...
```{r mfi composite figure enterics, fig.width = 10, fig.height = 5, warning = FALSE}
plot_mfi_comp_enterics <- grid.arrange(plot_mfi_means_enterics,plot_mfi_diff_enterics,ncol=2,nrow=1, widths = c(4,4))

ggsave(here("figures","mordor-ab-diff-mfi-enterics.png"),plot_mfi_comp_enterics,device = "png",width = 10, height = 3)
```
...
knitr::kable(mu_mfi_table %>% slice(12:20),
             col.names = c("Pathogen, antigen", "Placebo","Azithro.","Difference (95% CI)","P-value*","P-value†"), 
             digits = c(0,3,3,0,2,2),
             align = "lcccc") %>%
  kable_styling(bootstrap_options = c("striped","condensed"),full_width = TRUE) %>%
...
plot_mfi_comp_malaria <- grid.arrange(plot_mfi_means_malaria,plot_mfi_diff_malaria,ncol=2,nrow=1, widths = c(4,4))

ggsave(here("figures","mordor-ab-diff-mfi-malaria.png"),plot_mfi_comp_malaria,device = "png",width = 10, height = 3)
```
...
# within enterics and malaria
#-----------------------------
bh_p_enterics <- p.adjust(permute_serop$permute_p[1:7],method = "BH")
bh_p_malaria <- p.adjust(permute_serop$permute_p[8:16],method = "BH")

```

## Enterics seroprevalence table
```{r seroprev means table enterics}
...
knitr::kable(mu_serop_table %>% slice(1:7),
             col.names = c("Pathogen, antigen", "Placebo","Azithromycin","Difference (95% CI)","P-value*","P-value†"), 
             digits = 2,
             align = "lcccc") %>%
  kable_styling(bootstrap_options = c("striped","condensed"),full_width = TRUE) %>%
...
## Enterics seroprevalence figure

```{r seroprev figure enterics}
#------------------------------
...
plot_serop_comp_enterics <- grid.arrange(plot_serop_means_enterics,plot_serop_diff_enterics,ncol=2,nrow=1, widths = c(4,4))

ggsave(here("figures","mordor-ab-diff-seroprev-enterics.png"),plot_serop_comp_enterics,device = "png",width = 10, height = 2.5)
```


## Malaria seroprevalence table
```{r seroprev means table malaria}
knitr::kable(mu_serop_table %>% slice(8:16),
             col.names = c("Pathogen, antigen", "Placebo","Azithromycin","Difference (95% CI)","P-value*","P-value†"), 
             digits = 2,
             align = "lcccc") %>%
  kable_styling(bootstrap_options = c("striped","condensed"),full_width = TRUE) %>%
...
plot_serop_comp_malaria <- grid.arrange(plot_serop_means_malaria,plot_serop_diff_malaria,ncol=2,nrow=1, widths = c(4,4))

ggsave(here("figures","mordor-ab-diff-seroprev-malaria.png"),plot_serop_comp_malaria,device = "png",width = 10, height = 2.5)
```
...
# within enterics and malaria
#-----------------------------
bh_p_enterics <- p.adjust(foi_ests$hr_p[1:7],method = "BH")
bh_p_malaria <- p.adjust(foi_ests$hr_p[8:16],method = "BH")
...
knitr::kable(foi_ests_tab %>% select(antigenf,lambda0c,lambda1c,hrc,hr_p,bh_p) %>% slice(1:7),digits=3,
             ...
             align = "lcccc",
             col.names = c("Antigens","Placebo SCR (95% CI)","Azithromycin SCR (95% CI)", "HR (95% CI)","P-value*","P-value†"),
             row.names = FALSE)  %>%
...
plot_scr_means_enterics <- ggplot(data = mu_scr_plot %>% filter(group != "malaria"), aes(x = antigenf, color = arm)) +
  ...
  labs(x = NULL, y = "force of infection\n(seroconversion rate per child-year)", tag = "C") +
...
```{r scr hazard ratio figure enterics}
#------------------------------
...
```{r scr composite figure enterics, fig.width = 10, fig.height = 5, warning = FALSE}
plot_scr_comp <- grid.arrange(plot_scr_means_enterics,plot_scr_hr_enterics,ncol=2,nrow=1, widths = c(4,4))

ggsave(here("figures","mordor-ab-scr-hr-enterics.png"),plot_scr_comp,device = "png",width = 10, height = 2.5)
```
...
plot_scr_comp_malaria <- grid.arrange(plot_scr_means_malaria,plot_scr_hr_malaria,ncol=2,nrow=1, widths = c(4,4))

ggsave(here("figures","mordor-ab-scr-hr-malaria.png"),plot_scr_comp_malaria,device = "png",width = 10, height = 2.5)
```
