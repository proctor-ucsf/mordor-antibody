...
# make figure
pcols <- c(cbpal[-5],vircols[c(2,4,6,8)])
plot_mfi_means <- ggplot(data = mu_mfi_plot, aes(x = antigenf, color = pathogen, shape = arm)) +
  facet_grid(group~., scales = "free_y") +
  # plot points
  geom_errorbar(aes(ymin = mean_lb, ymax = mean_ub),width = 0.5, 
...
# make the figure
pcols <- c(cbpal[-5],vircols[c(2,4,6,8)])
plot_mfi_diff <- ggplot(data = mu_mfi_plot2, aes(x = antigenf, y = diff, color = pathogen)) +
  facet_grid(group~., scales = "free_y") +
  geom_hline(yintercept = 0, color = "gray60") +
  geom_errorbar(aes(ymin = diff_lb, ymax = diff_ub),width = 0.5) +
  geom_point() +
  scale_color_manual(values = pcols) +
