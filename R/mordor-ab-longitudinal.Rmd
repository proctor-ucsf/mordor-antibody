...
knitr::kable(irr_ests %>% select(antigenf,ni_a,nit_a,seroi_a,seroi_lb_a,seroi_ub_a,ni_p,nit_p,seroi_p,seroi_lb_p,seroi_ub_p,irr,irr_lb,irr_ub),
...
knitr::kable(irr_ests %>% select(antigenf,nr_a,nrt_a,seror_a,seror_lb_a,seror_ub_a,nr_p,nrt_p,seror_p,seror_lb_p,seror_ub_p,srr,srr_lb,srr_ub),
...
# Examine incident seroconversions from phase 0 to 6

The only measurements that straddle a wet season in the trial are between baseline (phase 0) and phase 6.   Limit the analysis to children who were measured at both these time points to assess the effect of the azithomycin on incident seroconversions over a wet season.

```{r limit data to phases 0 and 6}
#-----------------------------
# limit the dataset to children
# measured in both phases 0, 6
#-----------------------------
dl06 <- dl5 %>%
  group_by(childid,antigenf) %>%
  mutate(minphase = min(phase)) %>%
  filter(minphase == 0 & phase <=6) %>%
  mutate(phase6 = ifelse(phase==6,1,0),
         phase6 = max(phase6)) %>%
  filter(phase6==1)
...
# by antigen
#-----------------------------
table(dl06$antigenf,dl06$phase)

```

## Estimate rates by arm

Estimate prospective incidence rates and incidence rate ratio by arm between phases 0 and 6 (straddling a single wet season)

```{r estimate prospective rates by arm phases 0 and 6}
#-----------------------------
# estimate sero-incidence rates
# for antibody boosting and waning 
# by arm
...
## Summary table - seroconversion rates phases 0-6 months
```{r summary irr table phases 0 and 6}

knitr::kable(irr_ests %>% select(antigenf,ni_a,nit_a,seroi_a,seroi_lb_a,seroi_ub_a,ni_p,nit_p,seroi_p,seroi_lb_p,seroi_ub_p,irr,irr_lb,irr_ub),
             caption = "Seroconversion rates estimated longitudinally among children in MORDOR Niger, from baseline to the 6-month follow-up.",
             col.names = c("Pathogen, antigen", "N events","Person-Years","Rate per year","min 95", "max 95","N events","Person-Years","Rate per year","min 95", "max 95", "IRR", "min 95", "max 95"), 
             digits = 2,
             align = "lrrcccrrcccccc") %>%
  kable_styling(bootstrap_options = c("striped","condensed"),full_width = TRUE) %>%
