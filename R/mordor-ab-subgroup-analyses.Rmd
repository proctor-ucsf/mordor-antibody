...
Compare mean antibody levels and seroprevalence by pre-specified subgroups: trial phase (6, 12, 24, 36), child age (age 6 months or younger at the start of the trial vs older), and season (wet vs dry).  The rationale for these subgroups is specified in the pre-analysis plan: https://osf.io/d9s4t/
...
d_enteric_comps <- d2 %>%
  ...
  select(clusterid,phasef,arm,childid,agem,agem_start_under6,wet_start,wet_end,season,sex,dbsid,pathogen,antigenf,seropos)
...
mu_mfi_table <- mu_mfi2 %>%
  ...
  select(antigenf, phasef, control, azithro, diff_ci,intP)
...
knitr::kable(mu_mfi_table,
  ...
  kableExtra::group_rows(group_label = "Enterics and strep", start_row = 1, end_row = 44) %>%
  kableExtra::group_rows(group_label = "Malaria", start_row = 45, end_row = 80) %>%
  footnote(
...
knitr::kable(mu_serop_table,
  ...
  kableExtra::group_rows(group_label = "Enterics and strep", start_row = 1, end_row = 28) %>%
  kableExtra::group_rows(group_label = "Malaria", start_row = 29, end_row = 64) %>%
  footnote(
    symbol=c("P-value for additive interaction by study phase.")
  )
...
mu_mfi_table <- mu_mfi2 %>%
  ...
  mutate(diff_ci = paste0(sprintf("%1.3f",diff)," (",sprintf("%1.3f",diff_lb),", ", sprintf("%1.3f",diff_ub),")")) %>%
  select(antigenf, agem_start_under6, control, azithro, diff_ci,intP)
...
knitr::kable(mu_mfi_table,
  ...
  kable_styling(bootstrap_options = c("striped","condensed"),full_width = TRUE) %>%
  add_header_above(c(" " = 2, "Mean log10\nLuminex Response (MFI-bg)" = 2,  " " = 2)) %>%
  kableExtra::group_rows(group_label = "Enterics and strep", start_row = 1, end_row = 22) %>%
  kableExtra::group_rows(group_label = "Malaria", start_row = 23, end_row = 40) %>%
  footnote(
    symbol=c("P-value for additive interaction by study phase.")
  )
...
knitr::kable(mu_serop_table,
  ...
  kable_styling(bootstrap_options = c("striped","condensed"),full_width = TRUE) %>%
  add_header_above(c(" " = 2, "Proportion\nSeropositive" = 2,  " " = 2)) %>%
  kableExtra::group_rows(group_label = "Enterics and strep", start_row = 1, end_row = 14) %>%
  kableExtra::group_rows(group_label = "Malaria", start_row = 15, end_row = 32) %>%
  footnote(
    symbol=c("P-value for additive interaction by study phase.")
  )
...
We joined spatio-temporal gridded estimates of monthly precipitation to each community's location. Precipitation data were from:

Harris, I., Osborn, T. J., Jones, P. & Lister, D. Version 4 of the CRU TS monthly high-resolution gridded multivariate climate dataset. Sci Data 7, 109 (2020). https://pubmed.ncbi.nlm.nih.gov/32246091/ 

We found highly consistent estimated temporal rainfall patterns across the 30 study communities. Therefore, we have assigned a common wet season based on rainfall patterns in each year, defined as total monthly rainfall >4mm (cutoff chosen by visual inspection to capture all of the seasonally high rainfall months during the study period). The rainfall-defined wet season spanned from April (2016, 2017) or May (2015, 2018) through October (2015-2018) during the study period.


## MFI by season and arm

```{r means by arm and season}
#-----------------------------
# tally number of observations
# by antigen
#-----------------------------
n_mfi <- d2 %>%
  group_by(pathogen,antigenf,arm,season) %>%
  mutate(nomiss = ifelse(!is.na(logmfi),1,0)) %>%
  summarize(nobs = sum(nomiss), .groups = "keep") %>%
  pivot_wider(id_cols = c(pathogen, antigenf,season), names_from = "arm", values_from = nobs) %>%
  rename(control_n = sun,
         azithro_n = moon)
...
mu_mfi <- d2 %>%
  group_by(pathogen,antigenf,arm,season) %>%
  summarize(mean_mfi = mean(logmfi), .groups = "keep") %>%
  pivot_wider(id_cols = c(pathogen, antigenf,season), names_from = "arm", values_from = mean_mfi) %>%
  rename(control = sun,
         azithro = moon) %>%
  mutate(diff = azithro - control)
...
# merge means onto the Ns
#-----------------------------
mu_mfi <- left_join(n_mfi, mu_mfi, by=c("pathogen","antigenf","season"))
```



```{r bootstrap mean MFI diff by season}
#-----------------------------
# bootstrap resample communities
# with replacement to estimate
# 95% CIs for difference in means
...
ncl <- length(unique(d2$clusterid))
bsamp <- matrix(sample(unique(d2$clusterid),size = ncl*nreps, replace = TRUE),nrow = ncl,ncol = nreps)
d_mfi_boot <- d2 %>% select(clusterid,antigenf,season,arm,logmfi)
mu_mfi_boot <- foreach(bi = 1:nreps, .combine = rbind) %do% {
  ...
  di <- left_join(data.frame(clusterid = bsamp[,bi], stringsAsFactors = FALSE),
                  d_mfi_boot, by = "clusterid")
  di %>%
  group_by(antigenf,season,arm) %>%
  summarize(mean_mfi = mean(logmfi), .groups = "keep") %>%
  pivot_wider(id_cols = c("antigenf","season"), names_from = "arm", values_from = mean_mfi) %>%
  rename(control = sun,
         azithro = moon) %>%
  mutate(diff = azithro - control, 
         bootrep = bi)
...
```{r mean mfi bootstrap cis by season}
#-----------------------------
# calculate percentile 95%
# CIs from the bootstrap
# distribution merge to estimates
...
mu_mfi_ci <- mu_mfi_boot %>%
  ...
  mutate(mean_c_lb = quantile(control, probs = 0.025),
         ...
         diff_lb = quantile(diff, probs = 0.025),
         diff_ub = quantile(diff, probs = 0.975)) %>%
  select(antigenf,season,starts_with("mean_"), starts_with("diff_")) %>%
  slice(1)
  
mu_mfi2 <- mu_mfi %>%
  left_join(mu_mfi_ci, by = c("antigenf","season"))
...
```{r mfi p-values for interaction by season}
...
#-----------------------------
clmeans_mfi <- d2 %>% 
    group_by(antigenf,arm,season,clusterid) %>%
    summarize(mean_mfi = mean(logmfi), .groups = "keep")

mfi_season_Ftest <- foreach(abi = levels(d2$antigenf), .combine = rbind) %do% {
    # restrict to a particular antigen
    # summarize cluster level means by season and arm
    di <- clmeans_mfi %>% filter(antigenf == abi) 
  
    # ANOVA F-test on cluster level means
    # See Hayes and Moulten 2017, section 10.8 (p241)
    fiti <- lm(mean_mfi ~ arm*season, data=di)
    intFP <- as.numeric(anova(fiti)[3,4:5])
    res <- data.frame(antigenf=abi,season="wet season",intF=intFP[1],intP=intFP[2])
...
### Table of results
```{r MFI means by season group table}
options(knitr.kable.NA = '') 
mu_mfi_table <- mu_mfi2 %>%
  ungroup() %>%
  left_join(mfi_season_Ftest,by=c("antigenf","season")) %>%
  mutate(diff_ci = paste0(sprintf("%1.3f",diff)," (",sprintf("%1.3f",diff_lb),", ", sprintf("%1.3f",diff_ub),")")) %>%
  select(antigenf, season, control, azithro, diff_ci,intP)

knitr::kable(mu_mfi_table,
             col.names = c("Pathogen, antigen", "Season","Placebo","Azithro.","Difference (95% CI)","P-value*"), 
             digits = 3,
             align = "lcccr") %>%
  kable_styling(bootstrap_options = c("striped","condensed"),full_width = TRUE) %>%
  add_header_above(c(" " = 2, "Mean log10\nLuminex Response (MFI-bg)" = 2,  " " = 2)) %>%
...
```{r MFI diff season figure enterics}
#------------------------------
# plot the difference between
# arms by season
#------------------------------
...
mu_mfi_plot2 <- mu_mfi2 %>%
  ungroup() %>%
  mutate(season2 = ifelse(season == "wet season","wet","dry"),
         season2 = factor(season2))
...
# make the figure
pcols <- brewer.pal(9,"PuBuGn")[7:8]
plot_mfi_diff_season_enterics <- ggplot(data = mu_mfi_plot2 %>% filter(group!="malaria"), aes(x = season2, y = diff, color = season2, fill = season2)) +
  facet_wrap(antigenf~., ncol=4, nrow = 3) +
  geom_hline(yintercept = 0, color = "gray60") +
  geom_errorbar(aes(ymin = diff_lb, ymax = diff_ub),width = 0.2,alpha=0.6) +
  geom_point(alpha=0.6) +
  scale_y_continuous(breaks = seq(-0.6,0.6,by=0.3), labels = sprintf("%1.1f",seq(-0.6,0.6,by=0.3))) +
  scale_color_manual(values = pcols) +
  labs(x = "season", y = "difference in log10 Luminex response (MFI-bg)\nazithromycin - placebo", tag = "A") +
  coord_cartesian(ylim = c(-0.6,0.6)) +
  theme(
    legend.position = "none",
    panel.grid.minor = element_blank()
...
```{r MFI diff season figure malaria}
#------------------------------
# plot the difference between
# arms by age group
#------------------------------
pcols <- brewer.pal(9,"PuBuGn")[7:8]
plot_mfi_diff_season_malaria <- ggplot(data = mu_mfi_plot2 %>% filter(group=="malaria"), aes(x = season2, y = diff, color = season2, fill = season2)) +
  facet_wrap(antigenf~., ncol=3, nrow = 3) +
  geom_hline(yintercept = 0, color = "gray60") +
  geom_errorbar(aes(ymin = diff_lb, ymax = diff_ub),width = 0.2,alpha=0.6) +
  geom_point(alpha=0.6) +
...
```{r mfi diff season composite figure, fig.width = 10, fig.height = 6}
plot_mfi_season_comp <- grid.arrange(plot_mfi_diff_season_enterics,plot_mfi_diff_season_malaria,ncol=2,nrow=1, widths = c(1,0.75))

ggsave(here("figures","mordor-ab-diff-mfi-by-season.png"),plot_mfi_season_comp,device = "png",width = 12, height = 4)
```


## Seroprevalence by season and arm

```{r seroprev by arm season}
...
mu_serop <- d_serop %>%
  group_by(pathogen,antigenf,arm, season) %>%
  summarize(mean_serop = mean(seropos), .groups = "keep") %>%
  pivot_wider(id_cols = c(pathogen, antigenf, season), names_from = "arm", values_from = mean_serop) %>%
  rename(control = sun,
         azithro = moon) %>%
  mutate(diff = azithro - control)
...
# merge means onto the Ns
#-----------------------------
mu_serop <- left_join(n_serop, mu_serop, by=c("pathogen","antigenf", "season"))
```


```{r bootstrap mean seropos diff by season}
#-----------------------------
# bootstrap resample communities
# with replacement to estimate
# 95% CIs for difference in seroprev
...
ncl <- length(unique(d_serop$clusterid))
bsamp <- matrix(sample(unique(d_serop$clusterid),size = ncl*nreps, replace = TRUE),nrow = ncl,ncol = nreps)
d_serop_boot <- d_serop %>% select(clusterid,antigenf,arm,season,seropos)
mu_serop_boot <- foreach(bi = 1:nreps, .combine = rbind) %do% {
  ...
  di <- left_join(data.frame(clusterid = bsamp[,bi], stringsAsFactors = FALSE),
                  d_serop_boot, by = "clusterid")
  di %>%
  group_by(antigenf,arm,season) %>%
  summarize(mean_serop = mean(seropos), .groups = "keep") %>%
  pivot_wider(id_cols = c("antigenf","season"), names_from = "arm", values_from = mean_serop) %>%
  rename(control = sun,
         azithro = moon) %>%
  mutate(diff = azithro - control, 
         bootrep = bi)
...
```{r mean seroprev bootstrap cis by season}
#-----------------------------
# calculate percentile 95%
# CIs from the bootstrap
# distribution merge to estimates
...
mu_serop2 <- mu_serop %>%
  left_join(mu_serop_ci, by = c("antigenf","season"))
...
```{r seroprev p-values for interaction season}
...
serop_season_Ftest <- foreach(abi = levels(d_serop$antigenf), .combine = rbind) %do% {
    # restrict to a particular antigen
    # summarize cluster level means by phase and arm
    di <- clmeans_serop %>% filter(antigenf == abi) 
    ...
    # ANOVA F-test on cluster level means
    # See Hayes and Moulten 2017, section 10.8 (p241)
    fiti <- lm(seroprev ~ arm*season, data=di)
    intFP <- as.numeric(anova(fiti)[3,4:5])
    res <- data.frame(antigenf=abi,season="wet season",intF=intFP[1],intP=intFP[2])
...
### Table of results
```{r seroprev means by season table}
mu_serop_table <- mu_serop2 %>%
  ungroup() %>%
  left_join(serop_season_Ftest, by = c("antigenf","season")) %>%
  mutate(diff_ci = paste0(sprintf("%1.2f",diff)," (",sprintf("%1.2f",diff_lb),", ", sprintf("%1.2f",diff_ub),")")) %>%
  select(antigenf,season, control, azithro, diff_ci, intP)

knitr::kable(mu_serop_table,
             col.names = c("Pathogen, antigen","Season", "Placebo","Azithromycin","Difference (95% CI)","P-value*"), 
             digits = c(0,0,2,2,0,3),
             align = "lcccc") %>%
  kable_styling(bootstrap_options = c("striped","condensed"),full_width = TRUE) %>%
  add_header_above(c(" " = 2, "Proportion\nSeropositive" = 2,  " " = 2)) %>%
...
```{r serop diff season figure enterics}
#------------------------------
# plot the difference between
# arms by season
#------------------------------
...
# make the figure
pcols <- brewer.pal(9,"PuBuGn")[7:8]
plot_serop_diff_season_enterics <- ggplot(data = mu_serop_plot2 %>% filter(group!="malaria"), aes(x = season2, y = diff, color = season2, fill = season2)) +
  facet_wrap(antigenf~., ncol=3, nrow = 3) +
  geom_hline(yintercept = 0, color = "gray60") +
  geom_errorbar(aes(ymin = diff_lb, ymax = diff_ub),width = 0.2,alpha=0.6) +
  geom_point(alpha=0.6) +
  scale_y_continuous(breaks = seq(-0.3,0.3,by=0.1), labels = sprintf("%1.0f",100*seq(-0.3,0.3,by=0.1))) +
  scale_color_manual(values = pcols) +
  labs(x = "season", y = "difference in seroprevalence (%)\nazithromycin - placebo", tag = "A") +
  coord_cartesian(ylim = c(-0.2,0.2)) +
  theme(
    legend.position = "none",
    panel.grid.minor = element_blank()
...
```{r seroprev diff season figure malaria}
#------------------------------
# plot the difference between
# arms by phase
#------------------------------
pcols <- brewer.pal(9,"PuBuGn")[7:8]
plot_serop_diff_season_malaria <- ggplot(data = mu_serop_plot2 %>% filter(group=="malaria"), aes(x = season2, y = diff, color = season2, fill = season2)) +
  facet_wrap(antigenf~., ncol=3, nrow = 3) +
  geom_hline(yintercept = 0, color = "gray60") +
  geom_errorbar(aes(ymin = diff_lb, ymax = diff_ub),width = 0.2,alpha=0.6) +
  geom_point(alpha=0.6) +
...
```{r season composite figure, fig.width = 10, fig.height = 6}
plot_serop_comp_season <- grid.arrange(plot_serop_diff_season_enterics,plot_serop_diff_season_malaria,ncol=2,nrow=1, widths = c(1,1))

ggsave(here("figures","mordor-ab-diff-seroprev-by-season.png"),plot_serop_comp_season,device = "png",width = 13, height = 5)
```
