...
fit_age <- foreach(abi = levels(d3$antigenf), .combine = rbind) %do% {
  ...
  fit_age <- mgcv::gam(logmfi ~ s(agem,bs="cr") + 
                       ...
                       data=di)
  newd0 <- di %>%  mutate(dummy=0, arm = "sun")
  newd1 <- di %>%  mutate(dummy=0, arm = "moon")
...
fit_foi_age <- foreach(abi = levels(d_foi$antigenf), .combine = rbind) %do% {
  ...
  newd0 <- data.frame(antigenf = abi, agem=seq(min(di$agem),max(di$agem),by=0.01), dummy=0,clusteridf="C1",arm = "sun")
  newd1 <- data.frame(antigenf = abi, agem=seq(min(di$agem),max(di$agem),by=0.01), dummy=0,clusteridf="C1",arm = "moon")
...
fit_foi_comp_age <- foreach(abi = levels(d_foi_comp$antigenf), .combine = rbind) %do% {
  ...
  # estimate average seroconversion rate by arm
  newd0 <- data.frame(antigenf = abi, agem=seq(min(di$agem[di$estfoi==1]),max(di$agem[di$estfoi==1]),by=0.01), dummy=0,estfoi=1,clusteridf="C1",arm = "sun")
  newd1 <- data.frame(antigenf = abi, agem=seq(min(di$agem[di$estfoi==1]),max(di$agem[di$estfoi==1]),by=0.01), dummy=0,estfoi=1,clusteridf="C1",arm = "moon")
  foi_tr0_ci <- foiCI(m=fit_age,newdata=newd0,fdvar="agem",nreps=1000)
  foi_tr1_ci <- foiCI(m=fit_age,newdata=newd1,fdvar="agem",nreps=1000)
  foi_ci <- bind_rows(foi_tr0_ci,foi_tr1_ci)
...
pplabs <- fit_age_enterics %>%
  ...
  mutate(minX = 6, 
         maxX = ifelse(antigenf %in% c("Salmonella LPS B", "Salmonella LPS D","Streptococcus A"),60,24),
         arm = "sun"
         )
...
fitp_enterics <- fitp_age2 %>%
  ...
  mutate(antigenf = factor(antigenf,levels = c(levels(fitp_comp_age2$antigenf)[1],"ETEC LTB","Cholera CTB",levels(fitp_comp_age2$antigenf)[2:4],"Streptococcus A")))
...
d_shade_box <- data.frame(antigenf = levels(fitp_enterics$antigenf)) %>%
  mutate(minX = 6, 
         maxX = ifelse(antigenf %in% c("Salmonella LPS B or D","Streptococcus A"),60,24),
         arm = "sun"
         )
...
foip_enterics <- fit_foi_age %>%
  filter(antigenf %in% c("ETEC LTB","Cholera CTB","Streptococcus A")) %>%
  bind_rows(fit_foi_comp_age) %>%
  mutate(antigenf = factor(antigenf,levels = c(levels(d_foi_comp$antigenf)[1],"ETEC LTB","Cholera CTB",levels(d_foi_comp$antigenf)[2:4],"Streptococcus A")))
...
foip_malaria <- fit_foi_age %>%
  filter(antigenf %in% levels(fit_foi_age$antigenf)[12:20]) %>%
  mutate(antigenf = droplevels(antigenf))
