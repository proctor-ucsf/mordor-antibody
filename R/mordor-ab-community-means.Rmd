...
## Heatmap of seroprevalence by community
...
p <- ggplot(plot_mu_serop,aes(x=clrankf,y=antigenf,fill=seroprev*100)) +
  #facet over village
  facet_grid(group~arm,scales='free',space='free',switch="y")+
...
```{r plot seroprev by community, fig.width = 8, fig.height = 10}
pcols <- c(cbpal[2],vircols[3])
p_comm <- ggplot(data = mu_serop, aes(x = arm, y = seroprev, color = arm)) +
