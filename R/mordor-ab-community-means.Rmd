...
p <- ggplot(plot_mu_mfi,aes(x=clrankf,y=antigenf,fill=mulogmfi)) +
  ...
  labs(x="Clusters are stratified by arm then sorted by mean seroprevalence",y="",title="")+
  #remove extra space
...
ggsave(filename = here("figures","mordor-ab-community-seroprev-heatmap-enterics.png"),p, device = "png", width=6,height=2)
...
ggsave(filename = here("figures","mordor-ab-community-seroprev-heatmap-malaria.png"),p, device = "png", width=6,height=2.5)
...
p_comm <- ggplot(data = mu_serop %>% filter(group != "malaria" & antigenf != "Streptococcus A"), aes(x = arm, y = seroprev, color = arm)) +
...
ggsave(filename = here("figures","mordor-ab-community-seroprev-enterics.png"),p_comm, device = "png", width=6,height=4)
