...
d_pre2 <- d_pre %>%
  ...
  select(clusterid,pyear,wet_start,wet_end) %>%
  slice(1) %>%
  ungroup()
...
#-------------------------------
d11 <- d10 %>%
  select(clusterid,phase,arm,childid,agem,agem_start, agem_start_under6, sex, 
         testdate,
...
dtr2 <- dtr %>%
  ...
  # limit to relevant vars
  select(cluster_name,dupwpid_a,childid=masterperson, phase, agem = agex, female = femalex, trdate, treated, dose=dosex)
...
dchids <- d10 %>%
  ungroup() %>%
  select(childid, clusterid, morbidity_cluster_name = cluster_name, arm) %>%
  group_by(childid) %>%
  slice(1)
...
dtr_cnames <- dtr3 %>%
  group_by(clusterid) %>%
  select(clusterid, treatment_cluster_name = cluster_name, morbidity_cluster_name) %>%
  slice(1) 
...
# variables
dtr4 <- dtr3 %>%
  select(clusterid,arm,phase,childid,agem,female,trdate,treated,dose)
