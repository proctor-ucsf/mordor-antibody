...
#-------------------------------
pcols <- cbpal[c(2,6)]
plot_pre <- ggplot(data = dp %>% filter(clusterid=="C7"), aes(x = pdate, y = pre)) +
...
phase_test_months <- cl_test_months %>%
  group_by(phasef) %>%
  # drop cluster C64 in phase 12 — it was way off cycle
  mutate(dropobs = ifelse(clusterid == "C64" & phasef=="12",1,0)) %>%
  filter(dropobs == 0) %>%
