...
#-----------------------------
hbreaks <- seq(0,4.6,by=0.1)
plot_dist_under24mo <- ggplot(data = fit_age_enterics %>% filter(agem>12 & agem<=24), aes(x = logmfi)) +
  facet_grid(antigenf~.) +
  geom_histogram(aes(x = logmfi), breaks = hbreaks, color = NA, fill=pcol, alpha = 0.8)+
  geom_vline(data = dcuts, aes(xintercept = serocut),lwd = 0.25,col = "gray20") +
  scale_x_continuous(breaks = 0:4, labels = log10labs) +
  scale_y_continuous(expand = c(0,0), breaks = seq(0,700,by=100))+
  labs(x = NULL, y = "N observations", title = "Ages 12-24 mo", tag = "C") +
...
#-----------------------------
pcurve_composite_enterics <- grid.arrange(plot_age_mfi_enterics,plot_dist_under12mo,plot_dist_under24mo,plot_dist_24to59,ncol=4,nrow=1,widths=c(2.5,1.5,1.5,1.5))

ggsave(here("figures","mordor-ab-antibody-distributions-enterics.png"),pcurve_composite_enterics, device = "png", width = 12, height = 20)
...
```{r distribution malaria ages 12-24 mo}
#-----------------------------
# plot distribution among
# ages 12-24 months
#-----------------------------
hbreaks <- seq(0,4.6,by=0.1)
plot_dist_malaria_under24mo <- ggplot(data = fit_age_malaria %>% filter(agem>12 & agem<=24), aes(x = logmfi)) +
  facet_grid(antigenf~.) +
  geom_histogram(aes(x = logmfi), breaks = hbreaks, color = NA, fill=pcol, alpha = 0.8)+
  geom_vline(data = dcuts_malaria, aes(xintercept = serocut),lwd = 0.25,col = "gray20") +
  scale_x_continuous(breaks = 0:4, labels = log10labs) +
...
```{r distribution malaria 24-59 mo}
#-----------------------------
# plot distribution among
# ages 24-59 months
#-----------------------------
hbreaks <- seq(0,4.6,by=0.1)
plot_dist_malaria_24to59 <- ggplot(data = fit_age_malaria %>% filter(agem>24), aes(x = logmfi)) +
  facet_grid(antigenf~.) +
  geom_histogram(aes(x = logmfi), breaks = hbreaks, color = NA, fill=pcol, alpha = 0.8)+
  geom_vline(data = dcuts_malaria, aes(xintercept = serocut),lwd = 0.25,col = "gray20") +
  scale_x_continuous(breaks = 0:4, labels = log10labs) +
...
# into a composite figure
#-----------------------------
pcurve_composite_malaria <- grid.arrange(plot_age_mfi_malaria,plot_dist_malaria_under12mo,plot_dist_malaria_under24mo,plot_dist_malaria_24to59,ncol=4,nrow=1,widths=c(2.5,1.5,1.5,1.5))

ggsave(here("figures","mordor-ab-antibody-distributions-malaria.png"),pcurve_composite_malaria, device = "png", width = 12, height = 16)
