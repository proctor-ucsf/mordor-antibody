...
#-----------------------------
hbreaks <- seq(0,4.6,by=0.1)
plot_dist_malaria_under24mo <- ggplot(data = fit_age_malaria %>% filter(agem<=24), aes(x = logmfi)) +
  facet_grid(antigenf~.) +
  geom_histogram(aes(x = logmfi), breaks = hbreaks, color = NA, fill=pcol, alpha = 0.8)+
  geom_vline(data = dcuts_malaria, aes(xintercept = serocut),lwd = 0.25,col = "gray20") +
  scale_x_continuous(breaks = 0:4, labels = log10labs) +
  scale_y_continuous(expand = c(0,0), breaks = seq(0,700,by=100))+
  labs(x = NULL, y = "N observations", title = "Ages 0-24 mo", tag = "B") +
  coord_flip(ylim = c(0,700), xlim = c(0,4.5))+
  theme(
    legend.position = "none", 
    strip.text.y = element_blank()
...
# into a composite figure
#-----------------------------
pcurve_composite_malaria <- grid.arrange(plot_age_mfi_malaria,plot_dist_malaria_under12mo,plot_dist_malaria_under24mo,plot_dist_malaria_allages,ncol=4,nrow=1,widths=c(2.5,1.5,1.5,1.5))
